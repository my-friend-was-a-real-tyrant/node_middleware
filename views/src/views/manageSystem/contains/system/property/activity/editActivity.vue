...
<template>
    <div class="contain-main">
        ...
        <el-form ref="formData" :model="formData" :rules="formRules" class="scoped-form" label-width="150px" label-position="left" inline>
            <el-row>
                <el-col :span="12">
            ...
            <el-row>
                <el-col :span="24">
                    <el-form-item label="活动人数：" required prop="maxCount">
            ...
            <el-form-item label=" ">
                <el-button type="primary" size="small" @click="handleAddActivity('formData')">确定</el-button>
...
<script lang="babel">
    export default {
        ...
        methods: {
            ...
            handleAddActivity(params) {
                const _this = this;
                _this.$refs[params].validate((valid) => {
                    if (valid) {
                        _this.$http({
                            url: '/api/manageSystem/system/property/activity/editActivity',
                            method: 'patch',
                            data: _this.formData
                        }).then((response) => {
        ...
        mounted() {
            this.$http({
                ...
                if (response.data.code === '00000') {
                    for (let i = 0; i < response.data.activityInfo.imgList.length; i ++) {
                        const temp = {
                            name: 'content' + i,
                            url: response.data.activityInfo.imgList[i],
                            urlNoHostname: response.data.activityInfo.imgListNoHostname[i]
                        };
                        this.initImgList.push(temp);
                    }
                    this.formData.contact = response.data.activityInfo.contact;
                    this.formData.cellphone = response.data.activityInfo.cellphone;
                    this.formData.enrollEndDate = new Date(response.data.activityInfo.enrollEndDate);
                    this.formData.activityStartDatetime = new Date(response.data.activityInfo.activityStartDatetime);
                    this.formData.activityEndDatetime = new Date(response.data.activityInfo.activityEndDatetime);
                    this.formData.address = response.data.activityInfo.address;
                    this.formData.target = response.data.activityInfo.target;
                    this.formData.maxCount = response.data.activityInfo.maxCountNum;
                    this.formData.userCountMax = response.data.activityInfo.userCountMax;
                    this.formData.title = response.data.activityInfo.title;
                    this.formData.content = response.data.activityInfo.content;
                    this.formData.imgList = response.data.activityInfo.imgListNoHostname;
                }
            }).catch((response) => {
                console.log(response);
            });
