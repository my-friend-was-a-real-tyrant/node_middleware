    ...
    <div class="contain-main">
        ...
        <el-form ref="formData" :rules="formRules" :model="formData" label-width="120px" class="scoped-form" label-position="left">
            <el-form-item label="姓名">
                <span class="scoped-name">{{ managerName }}</span>
...
<script lang="babel">
    export default {
        ...
        methods: {
            handleResetPassword() {
                this.$http({
                    url: '/api/manageSystem/system/resetPassword',
                    method: 'patch',
                    data: {
            ...
            handleLockManager() {
                this.$http({
                    url: '/api/manageSystem/system/lockManager',
                    method: 'patch',
                    data: {
                        managerId: this.$route.query.managerId
                    }
            ...
            handleUnlockManager() {
                this.$http({
                    url: '/api/manageSystem/system/unlockManager',
                    method: 'patch',
                    data: {
                        managerId: this.$route.query.managerId
                    }
            ...
            handleSubmit() {
                ...
                this.$refs['formData'].validate((valid) => {
                    if (valid) {
                        _this.$http({
                            url: '/api/manageSystem/system/editManager',
                            method: 'patch',
                            data: {
                            ...
                            if (response.data.code === "00000") {
                                _this.$message('编辑成功');
                                _this.$router.push('/manageSystem/system/submanager');
                            }
                        }).catch((response) => {
                            console.log(response);
                        });
        ...
        mounted() {
             ...
             this.formData.managerCellphone = this.$route.query.managerCellphone;
             this.isLocked = this.$route.query.isLocked;
        }
    }
</script>